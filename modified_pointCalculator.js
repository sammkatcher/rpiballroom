...
for (var z = 0; z < huge_data.length; z++) {
	...
	// loops through Style (Standard/Smooth/etc) -> Dance (Waltz/Tango/etc) -> Level (Bronze/Silver/Gold)
    // Information taken from pointCalculator.php
...
function table_to_csv(pointTable) {
	var categories = ["Smooth", "Standard", "Rhythm", "Latin"];
    var dances = [ ["Waltz", "Tango", "Foxtrot", "VWaltz"], ["Waltz", "Quickstep", "Tango", "Foxtrot", "V. Waltz"], ["Cha Cha", "Rumba", "Swing", "Mambo", "Bolero"], ["Cha Cha", "Rumba", "Samba", "Jive", "Paso Doble"] ];

	var csv = "Category,Dance,Bronze,Silver,Gold<br>";
	...
	for (category in pointTable) {
		...
		for (dance in pointTable[category]) {
			...
			// Bronze - Silver - Gold
			for (var k = 0; k < pointTable[category][dance].length; k++) {
				csv += "," + (pointTable[category][dance][k]);
			}
			csv += "<br>";
		}
	}
