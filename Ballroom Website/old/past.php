...
if (phpCAS::isAuthenticated() && in_array(phpCAS::getUser(),$ADMIN_ALLOWED) ){
	...
	if ($result = $mysqli->query($query)) {
		...
		while( $row = $result->fetch_array(MYSQLI_NUM)){
			if($row[0]!='members' && $row[0] != 'login_attempts' && !in_array( current( explode("_",$row[0]) ), $displayed) ){
				...
				echo("<tr><td class='filename_cell'>".$budget_year."</td> <td class='button_cell'><form method=\"post\" onsubmit=\"return alert('The will save a copy of the budget and all transactions and will not delete it from the database.');\"><input type=\"submit\"  value=\"Archive\"><input type=\"hidden\"  name=\"archive\" value=\"".$row[0]."\"></form> </td></tr>");
			}
		}
		echo("</table>");
	}
	...
	foreach ($files as $file){
		...
		if ($file[0] != '.'){
			...
			echo("<tr><td class='filename_cell'>".$file."</td><td class='button_cell'><form method=\"post\" onsubmit=\"return confirm('Do you really want to restore this budget? Doing so will overwrite the current budget, including all transactions entered. Ensure that it has been archived before proceeding!');\"><input type=\"submit\"  value=\"Restore\"><input type=\"hidden\"  name=\"restore\" value=\"".$file."\"></form></td>");
			echo("<td class='button_cell'><form method=\"post\" onsubmit=\"return confirm('Do you really want to delete this archive? It will be gone forever!');\"><input type=\"submit\"  value=\"Delete\"><input type=\"hidden\"  name=\"delete\" value=\"".$file."\"></form></td>");
			echo("<td class='button_cell'><a href='archive/".$file."' download='archive/".$file."'> <img src='site_images/download.png' Title='Download' alt='Download'></a></td></tr>");
		}
	}
	echo("</table>");
	...
	foreach ($files as $file){
		...
		if ($file[0] != '.') {
			//print it to the user with the option to load the table to the database
			echo("<tr><td class='filename_cell'>".$file."</td></td>");
			echo("<td class='button_cell'><a href='excel_exports/".$file."' download='excel_exports/".$file."'> <img src='site_images/download.png' Title='Download' alt='Download'></a></td>");
			echo("<td class='button_cell'><form method='post' onsubmit=\"return confirm('Do you really want to delete this export? It will be gone forever!');\"><input type='image' alt='Delete?'  Title='Delete?' name='submit' src='site_images/remove.png' value='delete'><input type='hidden'  name='delete' value='".$file."'></form></td></tr>");
		}
	}
	echo("</table>");
	?>
