<?php 
	if(isset($_GET["album"])){
		...
		if(file_exists($path)){
			...
			preg_match($pattern, $album, $matches);					//separate parts of name into date and name
			$year = $matches[1];
			$month = $matches[2];
			$day = $matches[3];
...
<ul>
<?php
$filesnames = scandir($dir.$album);
			foreach ($filesnames as $file){
				if ($file != "." && $file != ".."){
					//echo('<a href="'.$path.'/'.$file.'"><img style="width: 200px;" src="'.$path.'/'.$file.'"></a>');
					echo('<li><img alt="" src="'.$path.'/'.$file.'" width="500" height="600" /></li>'); // add captions: <p>Dancing!</p>
				}
			}
?>
</ul>
