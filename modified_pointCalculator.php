...
<?php
  ...
  $lname = "";
  if (empty($_GET["fname"])) {
    echo "Error: Requires first name";
    exit;
  } else if (empty($_GET["lname"])) {
    echo "Error: Requires last name";
    exit;
  } else {
    $fname = $_GET["fname"];
    $lname = $_GET["lname"];
  ...
  $fname_list = explode(",", $fname);
  $lname_list = explode(",", $lname);

  if (count($fname_list) != count($lname_list)) {
	echo "Different number of first name and last names";
	exit;
  }
 
  $nameStrings = "";
  for ($i = 0; $i < count($fname_list)-1; $i++) {
	$nameStrings .= $fname_list[$i] . " " . $lname_list[$i] . ",";
  }
  $nameStrings .= $fname_list[$i] . " " . $lname_list[$i];
  echo $nameStrings;
  ...
  function printResults($some_names, $index) {
    $huge_results = array();
    for($i = 0; $i < count($some_names); $i++) {
      $fname = $some_names[$i][0];
      $lname = $some_names[$i][1];
      
      $startTime = microtime(true);
      echo "<h2>Results for <a href='" . getPersonO2cmUrl($fname, $lname) . "'>" . ucfirst($fname) . " " . ucfirst($lname) . "</a></h2>";

      echo "<a class='btn btn-info btn-ballroom' href='/modified_calculator'>Enter new name</a><br />";
	  ...
	  array_push($huge_results, $results);
    ...
    $huge_results = json_encode($huge_results);
  
    echo "<script type='text/javascript' src='modified_pointCalculator.js'></script>";
    echo "<script language='javascript'>createTable(" . $huge_results . ")</script>";
  }




  // Using modified pointCalculator: Input names with first and last separated by space (sorry Chang Ju! -CJ) and names themselves separated by a comma. 
  // If you want to let people with spaces use this (cough, Chang Ju), then just change nameStrings and the for loop to change how it's done. Still, you gotta change the pointCalculator code itself as well
  // Basically will run a for loop on the names and run printResults on the first/last combinations

  //$nameStrings = "Naya Murdock,Stephanya Moran,Sam Wiegand,Emi Ahlo,Skye Jacobson,Emily Parker,Hannah Powers,Jenna Hoggan,Harry Zhang,Jared Briones,Rebecca Mier,Eric Deiros,Emily Davey,Corrado Mazzarelli,Matthew Fox,Jordan Faas-Bush,Victoria Cervantes,Allison Daboval";
  $some_names = explode(",", $nameStrings);

  echo "" . print_r($some_names);

  for($i = 0; $i < count($some_names); $i++) {
    $some_names[$i] = explode(" ",$some_names[$i]);
  }

  printResults($some_names, $i);
?>