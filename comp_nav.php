<div class="text-center">
  <ul class="nav nav-pills pill-ballroom center-pills">
    <li id="comp_nav_home"><a href="/competition">Comp</a></li>
    <script>/*
    <li id="comp_nav_registration"><a href="/comp_registration">Registration</a></li>
    <li id="comp_nav_events"><a href="/comp_events">Events</a></li>
    <li id="comp_nav_schedule"><a href="/comp_schedule">Schedule</a></li>
    <!--<li class="disabled" id="comp_nav_workshops"><a href="/comp_other_events">Workshops</a></li>-->
    */</script>
    <li id="comp_nav_venue"><a href="/comp_venue">Venue</a></li>
    <li id="comp_nav_spectators"><a href="/comp_spectators">Spectators</a></li>
    <script>/*<li id="comp_nav_officials"><a href="/comp_judges">Officials</a></li>
    <li id="comp_nav_rules"><a href="/comp_rules">Rules</a></li>
    <li id="comp_nav_housing"><a href="/comp_housing">Housing</a></li>*/</script>
    <li id="comp_nav_sponsors"><a href="/comp_sponsors">Sponsors</a></li>
  </ul>
</div>
<br />
